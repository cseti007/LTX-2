...
class ValidationConfig(ConfigBaseModel):
    ...
    val_loss_seed: int = Field(
        default=12345,
        description=(
            "Seed for the noise generator used in val loss computation. The same seed produces the same "
            "noise across runs, so val loss differences reflect model changes, not noise variance."
        ),
    )
    ...
    def validate_val_loss_settings(self) -> "ValidationConfig":
        if self.compute_val_loss:
            if self.val_data_root is None:
                raise ValueError("val_data_root must be set when compute_val_loss=True")
            if not Path(self.val_data_root).exists():
                raise ValueError(f"val_data_root does not exist: {self.val_data_root}")
        return self
